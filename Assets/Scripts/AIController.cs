...
public class AIController : MonoBehaviour
{
    ...
    //Components
    KarapanMovement aiMovement;

    //Awake is called when the script instance is being loaded.
    void Awake()
    {
        aiMovement = GetComponent<KarapanMovement>();
    ...
    void FixedUpdate()
    {
        ...
        aiMovement.SetInputVector(inputVector);
    }
    ...
    float ApplyThrottleOrBrake(float inputX)
    {
        ...
        if (aiMovement.GetVelocityMagnitude() > maxSpeed)
            return 0;
    ...
    bool IsCarsInFrontOfAICar(out Vector3 position, out Vector3 otherCarRightVector)
    {
        ...
        RaycastHit2D raycastHit2d = Physics2D.CircleCast(transform.position + transform.up * 0.5f, 1.2f, transform.up, 6, 1 << LayerMask.NameToLayer("Karapan"));
